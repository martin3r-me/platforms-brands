...
class ListMoodboardBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.moodboard_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/moodboard_boards - Listet Moodboard/Bildsprache Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsMoodboardBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'images']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'images_count' => $board->images->count(),
                    'do_images_count' => $board->images->where('type', 'do')->count(),
                    'dont_images_count' => $board->images->where('type', 'dont')->count(),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'moodboard_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Moodboard(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Moodboards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Moodboards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'moodboard_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
