        ...
        <x-ui-page-actionbar :breadcrumbs="[
            ...
            <x-slot name="left">
                @can('update', $logoBoard)
                    <x-ui-button variant="ghost" size="sm" x-data @click="$dispatch('open-modal-logo-board-settings', { logoBoardId: {{ $logoBoard->id }} })">
            ...
            @can('update', $logoBoard)
                <x-ui-button variant="primary" size="sm" x-data @click="$dispatch('open-modal-logo-variant', { logoBoardId: {{ $logoBoard->id }} })">
                    @svg('heroicon-o-plus', 'w-4 h-4')
                    <span>Logo-Variante</span>
                </x-ui-button>
            @endcan
        </x-ui-page-actionbar>
    </x-slot>
