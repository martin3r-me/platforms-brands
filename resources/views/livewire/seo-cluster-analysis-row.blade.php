...
<div x-data="{ expanded: false }" class="group">
    ...
    <div @click="expanded = !expanded"
        ...
        <div class="pl-5 pr-4 py-3">
            ...
            <div class="hidden lg:flex items-center gap-1">
                ...
                {{-- Score --}}
                <div class="w-24 flex items-center justify-center gap-2">
            ...
            <div class="lg:hidden">
                ...
                <div class="mt-2.5 ml-9 grid grid-cols-2 sm:grid-cols-4 gap-x-4 gap-y-2">
                    ...
                    <div>
                        <div class="text-[10px] text-[var(--ui-muted)] uppercase tracking-wide">&Sigma; SV</div>
                    ...
                    <div>
                        ...
                        <div class="text-xs font-semibold text-emerald-700 tabular-nums mt-0.5">{{ number_format($data['traffic_value'], 0) }}&thinsp;&euro;</div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-show="expanded" x-collapse.duration.200ms>
        <div class="mt-1 ml-1 mr-1 rounded-b-xl border border-t-0 border-{{ $clusterColor }}-200/60 bg-gradient-to-b from-{{ $clusterColor }}-50/30 to-white overflow-hidden">

            {{-- Keyword table header (desktop) --}}
            <div class="hidden sm:grid grid-cols-[1fr_80px_60px_70px_50px_70px_70px] gap-1 px-5 py-2 bg-{{ $clusterColor }}-50/60 border-b border-{{ $clusterColor }}-100/60">
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700">Keyword</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-right">SV</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-right">KD</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-right">CPC</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-right">Pos</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-center">Intent</div>
                <div class="text-[10px] font-semibold uppercase tracking-wider text-{{ $clusterColor }}-700 text-center">Status</div>
            </div>
            ...
            <div class="divide-y divide-[var(--ui-border)]/20">
                @foreach($keywords as $kw)
                    ...
                    {{-- Desktop row --}}
                    <div class="hidden sm:grid grid-cols-[1fr_80px_60px_70px_50px_70px_70px] gap-1 items-center px-5 py-2 hover:bg-white/80 transition-colors">
                        <div class="text-xs font-medium text-[var(--ui-secondary)] truncate">{{ $kw->keyword }}</div>
                        <div class="text-xs tabular-nums text-[var(--ui-secondary)] text-right">{{ $kw->search_volume !== null ? number_format($kw->search_volume) : '&ndash;' }}</div>
                        <div class="flex justify-end">
                            @if($kwKd !== null)
                                <span class="inline-flex items-center px-1 py-0.5 rounded text-[10px] font-bold bg-{{ $kwKdColor }}-50 text-{{ $kwKdColor }}-700 border border-{{ $kwKdColor }}-200 tabular-nums" title="{{ $kwKdLabel }}">
                                    {{ $kwKd }}
                                </span>
                            @else
                                <span class="text-xs text-[var(--ui-muted)]">&ndash;</span>
                        ...
                        <div class="text-xs tabular-nums text-[var(--ui-secondary)] text-right">{{ $kw->cpc_cents !== null ? number_format($kw->cpc_cents / 100, 2) . ' &euro;' : '&ndash;' }}</div>
                        <div class="flex justify-end">
                            @if($kwPos !== null)
                                <span class="inline-flex items-center justify-center min-w-[22px] h-4 px-1 rounded text-[10px] font-bold bg-{{ $kwPosColor }}-50 text-{{ $kwPosColor }}-700 border border-{{ $kwPosColor }}-200 tabular-nums">{{ $kwPos }}</span>
                            @else
                                <span class="text-xs text-[var(--ui-muted)]">&ndash;</span>
                            @endif
                        </div>
                        <div class="flex justify-center">
                            @if($kwIntent)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-{{ $kwIntent['color'] }}-50 text-{{ $kwIntent['color'] }}-700 border border-{{ $kwIntent['color'] }}-200">{{ $kwIntent['label'] }}</span>
                            @else
                                <span class="text-xs text-[var(--ui-muted)]">&ndash;</span>
                            @endif
                        </div>
                    ...
                    <div class="sm:hidden px-4 py-2.5">
                        <div class="flex items-center justify-between gap-2">
                            <span class="text-xs font-medium text-[var(--ui-secondary)] truncate">{{ $kw->keyword }}</span>
                            <span class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[9px] font-medium bg-{{ $kwStatus['color'] }}-50 text-{{ $kwStatus['color'] }}-700 border border-{{ $kwStatus['color'] }}-200 flex-shrink-0">
                                <span class="w-1 h-1 rounded-full bg-{{ $kwStatus['color'] }}-500"></span>
                                {{ $kwStatus['label'] }}
                            </span>
                        </div>
                        <div class="mt-1.5 flex flex-wrap items-center gap-2 text-[10px]">
                            @if($kw->search_volume !== null)
                                <span class="text-[var(--ui-muted)]">SV <strong class="text-[var(--ui-secondary)]">{{ number_format($kw->search_volume) }}</strong></span>
                            @endif
                            @if($kwKd !== null)
                                <span class="inline-flex items-center px-1 py-0.5 rounded font-bold bg-{{ $kwKdColor }}-50 text-{{ $kwKdColor }}-700 border border-{{ $kwKdColor }}-200 tabular-nums">KD {{ $kwKd }}</span>
                            @endif
                            @if($kw->cpc_cents !== null)
                                <span class="text-[var(--ui-muted)]">CPC <strong class="text-[var(--ui-secondary)]">{{ number_format($kw->cpc_cents / 100, 2) }} &euro;</strong></span>
                            @endif
                            @if($kwPos !== null)
                                <span class="inline-flex items-center px-1 py-0.5 rounded font-bold bg-{{ $kwPosColor }}-50 text-{{ $kwPosColor }}-700 border border-{{ $kwPosColor }}-200 tabular-nums">#{{ $kwPos }}</span>
                            @endif
                            @if($kwIntent)
                                <span class="inline-flex items-center px-1.5 py-0.5 rounded-full font-medium bg-{{ $kwIntent['color'] }}-50 text-{{ $kwIntent['color'] }}-700 border border-{{ $kwIntent['color'] }}-200">{{ $kwIntent['label'] }}</span>
                            @endif
                        </div>
                    </div>
                @endforeach
            ...
            {{-- Footer summary --}}
            <div class="flex items-center justify-between px-5 py-2 bg-{{ $clusterColor }}-50/40 border-t border-{{ $clusterColor }}-100/60">
                <span class="text-[10px] text-{{ $clusterColor }}-600">{{ $data['count'] }} Keywords</span>
                <div class="flex items-center gap-3 text-[10px] text-{{ $clusterColor }}-600">
                    <span>&Sigma; SV: <strong>{{ number_format($data['sum_sv']) }}</strong></span>
                    <span>Wert: <strong>{{ number_format($data['traffic_value'], 0) }}&thinsp;&euro;</strong></span>
                    <span>Coverage: <strong>{{ $data['coverage'] }}%</strong></span>
                </div>
            </div>
        </div>
    </div>
