...
class ListTypographyBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.typography_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/typography_boards - Listet Typografie Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsTypographyBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'entries']);
            ...
            return ToolResult::success([
                'typography_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Typografie Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Typografie Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Typografie Boards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'typography_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
