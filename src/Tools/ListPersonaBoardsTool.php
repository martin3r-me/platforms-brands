...
class ListPersonaBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.persona_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/persona_boards - Listet Persona/Zielgruppen Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsPersonaBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'personas']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'personas_count' => $board->personas->count(),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'persona_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Persona Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Persona Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Persona Boards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'persona_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
