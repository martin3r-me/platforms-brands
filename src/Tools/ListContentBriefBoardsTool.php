...
class ListContentBriefBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.content_brief_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/content_brief_boards - Listet Content Brief Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsContentBriefBoard::query()
                ...
                ->with(['brand', 'user', 'team', 'seoBoard']);

            $this->applyStandardFilters($query, $arguments, [
                'name', 'description', 'content_type', 'search_intent', 'status', 'done', 'created_at', 'updated_at'
            ]);
            $this->applyStandardSearch($query, $arguments, ['name', 'description', 'target_slug']);
            $this->applyStandardSort($query, $arguments, [
                'name', 'content_type', 'search_intent', 'status', 'created_at', 'updated_at', 'order'
            ], 'order', 'asc');
            ...
            return ToolResult::success([
                'content_brief_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Content Brief Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Content Brief Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Content Brief Boards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'content_brief_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
