<x-ui-page>
    <x-slot name="navbar">
        <x-ui-page-navbar :title="$block->name" icon="heroicon-o-document-text">
            <x-slot name="actions">
                <a href="{{ route('brands.content-boards.show', $contentBoard) }}" class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors">
                    @svg('heroicon-o-arrow-left', 'w-4 h-4')
                    <span>Zurück zum Content Board</span>
                </a>
            </x-slot>
        </x-ui-page-navbar>
    </x-slot>
            ...
            <div class="p-4 space-y-6">
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-2">
                        <a href="{{ route('brands.content-boards.show', $contentBoard) }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors rounded-lg border border-[var(--ui-border)]/40 hover:bg-[var(--ui-muted-5)]">
                            @svg('heroicon-o-arrow-left', 'w-4 h-4')
                            <span>Zurück zum Content Board</span>
                        </a>
                    </div>
                ...
                {{-- Navigation zu anderen Blöcken --}}
                <div>
                    <h3 class="text-xs font-semibold uppercase tracking-wide text-[var(--ui-muted)] mb-3">Andere Text-Blöcke</h3>
                    <div class="flex flex-col gap-2">
                        @if($previousBlock)
                            <a href="{{ route('brands.content-board-blocks.show', ['brandsContentBoardBlock' => $previousBlock->id, 'type' => 'text']) }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors rounded-lg border border-[var(--ui-border)]/40 hover:bg-[var(--ui-muted-5)]">
                                @svg('heroicon-o-arrow-up', 'w-4 h-4')
                                <span class="truncate">{{ $previousBlock->name }}</span>
                            </a>
                        @endif
                        @if($nextBlock)
                            <a href="{{ route('brands.content-board-blocks.show', ['brandsContentBoardBlock' => $nextBlock->id, 'type' => 'text']) }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors rounded-lg border border-[var(--ui-border)]/40 hover:bg-[var(--ui-muted-5)]">
