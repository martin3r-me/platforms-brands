...
class ListLogoBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.logo_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/logo_boards - Listet Logo Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsLogoBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'variants']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'variants_count' => $board->variants->count(),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'logo_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Logo Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Logo Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Logo Boards: ' . $e->getMessage());
        }
    }
