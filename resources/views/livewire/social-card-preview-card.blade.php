...
<x-nx-kanban-card
    :title="''"
    :sortable-id="$card->id"
    :href="route('brands.social-cards.show', $card)"
>
    <!-- Titel -->
    <div class="mb-3">
    ...
    <!-- Body MD (Markdown Preview - truncated) -->
    @if($card->body_md)
        <div class="my-1.5 mb-3 line-clamp-3 text-xs text-[color:var(--nx-muted)]">
            {{ Str::limit(strip_tags($card->body_md), 120) }}
        </div>
    @endif

    <!-- Description (interner Kommentar) -->
    @if($card->description)
        <div class="my-1.5 mb-3 line-clamp-2 text-xs italic text-[color:var(--nx-faint)]">
