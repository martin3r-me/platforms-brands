...
class ListAssetBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.asset_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/asset_boards - Listet Asset Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsAssetBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'assets']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'assets_count' => $board->assets->count(),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'asset_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Asset Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Asset Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Asset Boards: ' . $e->getMessage());
        }
    }
