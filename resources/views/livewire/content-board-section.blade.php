<x-ui-page>
    <x-slot name="navbar">
        <x-ui-page-navbar :title="$section->name" icon="heroicon-o-squares-2x2">
            <x-slot name="actions">
                <a href="{{ route('brands.content-boards.show', $section->contentBoard) }}" class="inline-flex items-center gap-2 px-3 py-1.5 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors">
                    ...
                    <div class="p-4">
                        ...
                        @else
                            <div class="text-center py-8 border-2 border-dashed border-[var(--ui-border)]/40 rounded-lg bg-[var(--ui-muted-5)]">
                                <p class="text-sm text-[var(--ui-muted)] mb-3">Noch keine Blöcke in dieser Row</p>
                                @can('update', $section->contentBoard)
                                    <x-ui-button 
                                        variant="primary" 
                                        size="sm" 
                                    ...
                                    >
                                        <span class="inline-flex items-center gap-2">
                                            @svg('heroicon-o-plus', 'w-4 h-4')
                                            <span>Ersten Block hinzufügen</span>
                                        </span>
                                    </x-ui-button>
                                @endcan
                            </div>
    ...
    <x-slot name="sidebar">
        <x-ui-page-sidebar title="Section-Übersicht" width="w-80" :defaultOpen="true">
            <div class="p-6 space-y-6">
                {{-- Navigation --}}
                <div>
                    <h3 class="text-xs font-semibold uppercase tracking-wide text-[var(--ui-muted)] mb-3">Navigation</h3>
                    <div class="flex flex-col gap-2">
                        <a href="{{ route('brands.content-boards.show', $section->contentBoard) }}" class="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-[var(--ui-secondary)] hover:text-[var(--ui-primary)] transition-colors rounded-lg border border-[var(--ui-border)]/40 hover:bg-[var(--ui-muted-5)]">
                            @svg('heroicon-o-arrow-left', 'w-4 h-4')
                            <span>Zurück zum Content Board</span>
                        </a>
                    </div>
