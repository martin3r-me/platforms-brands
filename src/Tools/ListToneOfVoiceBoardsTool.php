...
class ListToneOfVoiceBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.tone_of_voice_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/tone_of_voice_boards - Listet Tone of Voice Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsToneOfVoiceBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'entries', 'dimensions']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'entries_count' => $board->entries->count(),
                    'dimensions_count' => $board->dimensions->count(),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'tone_of_voice_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Tone of Voice Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Tone of Voice Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Tone of Voice Boards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'tone_of_voice_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
