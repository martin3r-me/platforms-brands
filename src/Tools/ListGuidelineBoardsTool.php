...
class ListGuidelineBoardsTool implements ToolContract, ToolMetadataContract
{
    use HasStandardGetOperations;

    public function getName(): string
    {
        return 'brands.guideline_boards.GET';
    }

    public function getDescription(): string
    {
        return 'GET /brands/{brand_id}/guideline_boards - Listet Guidelines Boards einer Marke auf. REST-Parameter: brand_id (required, integer) - Marken-ID. filters (optional, array) - Filter-Array. search (optional, string) - Suchbegriff. sort (optional, array) - Sortierung. limit/offset (optional) - Pagination.';
    }
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            $query = BrandsGuidelineBoard::query()
                ->where('brand_id', $brandId)
                ->with(['brand', 'user', 'team', 'chapters.entries']);
            ...
            $boardsList = $boards->map(function ($board) {
                return [
                    ...
                    'chapters_count' => $board->chapters->count(),
                    'entries_count' => $board->chapters->sum(fn($ch) => $ch->entries->count()),
                    'done' => $board->done,
            ...
            return ToolResult::success([
                'guideline_boards' => $boardsList,
                'count' => count($boardsList),
                'brand_id' => $brandId,
                'brand_name' => $brand->name,
                'message' => count($boardsList) > 0
                    ? count($boardsList) . ' Guidelines Board(s) gefunden für Marke "' . $brand->name . '".'
                    : 'Keine Guidelines Boards gefunden für Marke "' . $brand->name . '".'
            ]);
        } catch (\Throwable $e) {
            return ToolResult::error('EXECUTION_ERROR', 'Fehler beim Laden der Guidelines Boards: ' . $e->getMessage());
        }
    }
    ...
    public function getMetadata(): array
    {
        return [
            'category' => 'query',
            'tags' => ['brands', 'guideline_board', 'list'],
            'read_only' => true,
            'requires_auth' => true,
            'requires_team' => false,
            'risk_level' => 'safe',
